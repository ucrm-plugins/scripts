...
# Fixes for WSL
sed -i "s#compose -v | sed 's/.*version \\\(#compose -v | sed 's/.*version v\\\?\\\(#" /tmp/unms-install/install-full.sh
sed -i "s#\(dockerRootDir=.*\)#\1\n  \[ -d \"\$\{dockerRootDir\}-desktop\" \] \&\& dockerRootDir+=\"-desktop\"#" /tmp/unms-install/install-full.sh
# End of fixes
